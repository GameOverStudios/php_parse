...
def get_chroma_client(path_chromadb):
    settings = Settings(
        is_persistent=True,
        persist_directory=path_chromadb
    )
...
path_chromadb = "chroma"

#import_all_tables_and_jsons_to_chromadb(output_directory)

# Após a importação
list_collections(get_chroma_client(path_chromadb))
