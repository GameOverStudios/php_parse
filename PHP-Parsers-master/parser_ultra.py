...
# github.com/JameelNabbo/PHP-Parsers/
...
class CustomVisitor(Visitor):
    ...
    def format_complex_attribute(self, attr_value, visited=None):
        ...
        protegendo contra loops infinitos e JSON incompleto.
        """
        if visited is None:
            visited = set()

        # Evitar recursões infinitas
        if id(attr_value) in visited:
            return f"<Referência recursiva a {type(attr_value).__name__}>"
        visited.add(id(attr_value))

        # Se for uma lista ou tupla, aplica recursividade em cada item
        if isinstance(attr_value, (list, tuple)):
            return [self.format_complex_attribute(item, visited.copy()) for item in attr_value]
        
        # Se for um dicionário, formata cada chave/valor recursivamente
        elif isinstance(attr_value, dict):
            return {key: self.format_complex_attribute(value, visited.copy()) for key, value in attr_value.items()}
        
        # Se for um objeto com atributos, percorre e formata seus atributos
        elif hasattr(attr_value, '__dict__'):
            formatted_dict = {}
            for key, value in attr_value.__dict__.items():
                if not key.startswith('_'):
                    if key == 'default':  # Renomear campo 'default' para 'default_value'
                        key = 'default_value'
                    if key == 'end':  # Renomear campo 'end' para 'end_value'
                        key = 'end_value'
                    formatted_dict[key] = self.format_complex_attribute(value, visited.copy())
            return formatted_dict
        
        # Se o valor tem o método 'accept', provavelmente é um nó do AST, visita recursivamente
        elif hasattr(attr_value, 'accept'):
            attr_value.accept(self)
            return self.get_node_info(attr_value)

        # Valor simples (int, string, etc.) ou desconhecido, retorna diretamente
